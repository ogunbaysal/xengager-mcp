...
function handleResponse(result: { status: number; body: any } | any): any {
  // When page.evaluate throws (network error etc.), it propagates as-is
...
export class TwitterGraphQLClient {
  ...
  async fetchMe(): Promise<GqlUserProfile> {
    ...
    if (data?.users?.[0]?.screen_name) {
      screenName = data.users[0].screen_name;
    } else if (Array.isArray(data) && data[0]?.user?.screen_name) {
      screenName = data[0].user.screen_name;
    }

    if (!screenName) throw new TwitterAPIError(0, "Could not determine current user");
